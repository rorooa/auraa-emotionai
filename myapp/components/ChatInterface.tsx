...
export default function ChatInterface({ 
    ...
    useEffect(() => {
        if (initialHistory.length > 0) {
            ...
            setHistory([{ role: 'ai', text: `Hi ${userName}! I'm AURAA. How are you feeling today?` }]);
        }
    }, [initialHistory, userName]);
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center p-6 pointer-events-none font-jura">

            {/* Chat History Section - "Other section for text" */}
            <div 
                ref={scrollRef}
                className="w-full max-w-lg mb-8 space-y-6 max-h-[60vh] overflow-y-auto pointer-events-auto px-4 custom-scrollbar flex flex-col pt-4"
            >
                {history.map((msg, i) => (
                    <div
                        key={i}
