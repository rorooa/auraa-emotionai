...
import { Send, Mic, History, Loader2 } from "lucide-react";
...
export default function ChatOverlay({ onChat, initialHistory = [], proactiveEmotion, onProactiveHandled, onLoadHistory }: ChatOverlayProps) {
    ...
    useEffect(() => {
        if (proactiveEmotion && onProactiveHandled) {
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto mb-6 custom-scrollbar pr-4 space-y-6">
                <AnimatePresence>
                    {history.map((msg, i) => (
                        ...
                        >
                            <div className={`p-5 rounded-[1.5rem] max-w-[85%] text-[15px] tracking-wide leading-relaxed shadow-lg backdrop-blur-md ${
                                msg.role === 'user' 
                    ...
                    {isThinking && (
                        ...
                        >
                             <div className="p-5 rounded-[1.5rem] bg-[#050508]/60 border border-white/10 text-white/50 text-xs uppercase tracking-widest flex items-center gap-3 rounded-tl-sm shadow-inner">
